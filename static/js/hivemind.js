...
JarbasHiveMind.prototype._onWsMessage = async function (event) {
    ...
    if (this._state < States.READY && !keyEstablished) {
        ...
        try {
            ...
            // A handshake step that throws must not escape the socket handler as
            // an unhandled rejection, which ends a Node process. It must also not
            // clear the guard: with no key assigned, a cleared guard would let
            // the next frame start a fresh handshake on this connection. The
            // guard stays set, the connection is closed, and connect() resets it.
            console.error('HiveMind: handshake step failed', e);
            this.onHiveError(new Error('HiveMind: the handshake failed; reconnect to try again'));
...
JarbasHiveMind.prototype._handleServerHandshake = async function (payload) {
    if (payload.noise && payload.noise.msg && this._noiseHandshake) {
        ...
        if (this._noiseHandshake || !this._handshake || !this._handshake.iv) {
            this.onHiveError(new Error('HiveMind: refused a legacy handshake response that this client did not ask for'));
            return;
        }
        ...
        if (typeof payload.envelope !== 'string' || !/^[0-9a-fA-F]{48,80}$/.test(payload.envelope)) {
            this.onHiveError(new Error('HiveMind: refused a malformed legacy handshake envelope'));
            return;
        }
